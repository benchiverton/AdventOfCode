...
var compartmentItems = backpacks.Select(x => (x.Take(x.Length / 2), x.Skip(x.Length / 2)));
...
var scores = compartmentItems.Select(ci => ci.Item1.First(i => ci.Item2.Contains(i))).Select(si => priorities[si]);
...
var count = 0;
foreach(var backpackContents in backpacks)
{
    if(count % 3 == 0)
    {
        groups.Add(new List<string>());
    }
    groups.Last().Add(backpackContents);
    count++;
}

var scores2 = groups.Select(g => g[0].First(c => g[1].Contains(c) && g[2].Contains(c))).Select(c => priorities[c]); ;
